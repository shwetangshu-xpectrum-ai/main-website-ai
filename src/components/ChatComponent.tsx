...
const ChatComponent: React.FC<ChatComponentProps> = ({ assistantName = "Xpectrum Assistant" }) => {
  ...
  useEffect(() => {
    if (textareaRef.current) {
      textareaRef.current.style.height = "45px"; // Reset height
  ...
  const handleFileChange = (event: ChangeEvent<HTMLInputElement>) => {
    if (event.target.files && event.target.files[0]) {
      ...
      // Optional: Show a preview for images
  ...
  const sendMessage = async () => {
    ...
    setSearchQuery(query || "Analyzing attachment...");

    let filePayload: { name: string; content: string; type: string }[] = [];
    ...
    if (attachedFile) {
      try {
        ...
        await new Promise<void>((resolve, reject) => {
          reader.onload = () => {
            try {
              ...
              const base64Content = result.split(',')[1]; // Get base64 part
              ...
              setTimeout(() => {
                ...
                setUploadProgress(0);
              }, 500); // Give a small delay to show 100% complete
        ...
        setMessages((prev) => [
          ...prev,
          { type: "error", content: "Failed to attach file. Please try again." }
        ]);
    ...
    // Properly structure the payload with the file
    const payload = {
      inputs: {},
      query: query,
    ...
    if (attachedFile) {
        userMessageContent += attachedFile.type.startsWith('image/') 
          ? `\n(Attached image: ${attachedFile.name})`
          : `\n(Attached file: ${attachedFile.name})`;
    }
    setMessages((prev) => [...prev, { type: "user", content: userMessageContent }]);
    setQuery("");
    setAttachedFile(null); // Reset attached file state

    try {
      // After a short delay, show "Generating answer for you..." status
      setTimeout(() => {
        setSearchQuery(null);
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    if (e.key === "Enter" && !e.shiftKey) {
      ...
      if (!isLoading && (query.trim() || attachedFile)) {
        sendMessage();
      }
    }
  };
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 sm:bottom-6 sm:right-6 md:bottom-8 md:right-8">
      ...
      >
        ...
        /> */}
        <MessageSquare size={28} /> {/* Replaced logo with chat icon */}
      </button>
      ...
      {isOpen && (
        <div 
          ...
          className={`absolute bottom-20 right-0 rounded-3xl 
            ...
            h-[75vh] max-h-[700px] sm:h-[600px] md:h-[650px] lg:h-[700px]
        ...
        >
          ...
          {/* Input Area - Using textarea instead of input for better multiline handling */}
          <div className="p-3 border-t border-gray-200 bg-white rounded-b-3xl">
            <div className="flex items-end bg-gray-50 rounded-xl border border-gray-200 p-1 relative">
              {/* Attachment button */}
              <div className="relative attachment-area">
                <button
              ...
              {/* Link Input Modal */}
              {showLinkInput && (
                <div className="absolute bottom-full left-0 mb-2 w-full bg-white rounded-md p-3 z-10 shadow-lg border border-gray-200 link-input-container">
              ...
              <input
                ...
                className="hidden"
                accept="image/*,.pdf,.doc,.docx,.txt" // Optional: restrict file types
              />

              {/* Display attached file name with preview for images */}
              {attachedFile && (
                <div className="text-xs px-2 flex items-center gap-1 bg-xpectrum-lightpurple/30 rounded-md py-1 mr-2">
                  {attachedFile.type.startsWith('image/') ? (
                    <div className="flex items-center">
                      <div className="w-6 h-6 mr-1 rounded overflow-hidden border border-gray-300">
                        <img 
                      ...
                      <span className="text-gray-700 max-w-[100px] truncate">{attachedFile.name}</span>
                    </div>
                  ) : (
                    <>
                      <Paperclip size={12} className="text-xpectrum-purple" />
              ...
              {isUploading && (
                <div className="text-xs px-2 flex items-center gap-2 bg-xpectrum-lightpurple/30 rounded-md py-1 mr-2">
                  <Loader size={12} className="text-xpectrum-purple animate-spin" />
                  ...
                  <span className="text-gray-700 min-w-[30px]">{uploadProgress}%</span>
                </div>
              )}
